...
- (void) initialize
{
   ...
   // [self setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"img_download.png"]]];
...
- (void) initalizeImageCounter
{
    ...
    _imageCounterBackground.alpha = 1.0f;
    _imageCounterBackground.layer.cornerRadius = 0.0f;
...
- (void) initializePageControl
{
    ...
    _pageControl.center = CGPointMake(_scrollView.frame.size.width / 2, _scrollView.frame.size.height-10);
...
-(void)previous{
    ...
    previousPage = _pageControl.currentPage - 1;
    
    [_scrollView scrollRectToVisible:CGRectMake(self.frame.size.width * previousPage, 0, self.frame.size.width, self.frame.size.width) animated:YES];
    [_pageControl setCurrentPage:previousPage];
    
    [self updateCaptionLabelForImageAtIndex:previousPage];
    
    if (self.slideshowShouldCallScrollToDelegate) {
        [self fireDidScrollToIndexDelegateForPage:previousPage];
    }
}
...
-(void) imageWithUrl:(NSURL*)url completion:(KIImagePagerImageRequestBlock)completion
{
    ...
    if (data) {
        ...
        if(completion) completion([UIImage imageWithData:data],nil);
    }
    else {

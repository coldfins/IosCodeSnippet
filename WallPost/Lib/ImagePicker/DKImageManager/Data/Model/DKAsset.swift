...
public extension DKAsset {
     ...
     */
    public func writeImageToFile(_ path: String, completeBlock: @escaping (_ success: Bool, _ url : URL) -> Void) {
		let options = PHImageRequestOptions()
		options.version = .current
		...
		getImageManager().fetchImageDataForAsset(self, options: options, completeBlock: { (data, _) in
			DKAssetWriter.writeQueue.addOperation({
				if let imageData = data {
					...
					completeBlock(true,URL(fileURLWithPath: path))
				} else {
					completeBlock(false,URL(fileURLWithPath: path))
				}
			})
		})
	}
     ...
     */
	public func writeAVToFile(_ path: String, presetName: String, completeBlock: @escaping (_ success: Bool, _ url : URL) -> Void) {
            ...
            DKAssetWriter.writeQueue.addOperation({
                if let avAsset = avAsset,
                    ...
                    exportSession.exportAsynchronously(completionHandler: {
                        completeBlock(exportSession.status == .completed ? true : false,URL(fileURLWithPath: path))
                    })
                } else {
                    completeBlock(false,URL(fileURLWithPath: path))
                }

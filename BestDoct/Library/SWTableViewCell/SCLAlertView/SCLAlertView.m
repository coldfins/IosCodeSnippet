...
- (void)setupViewWindowWidth:(CGFloat)windowWidth
{
    ...
    _customViewColor = [UIColor colorWithRed:84.0/255.0 green:110.0/255.0 blue:122.0/255.0 alpha:1.0];
}
...
- (void)handleTap1:(UIGestureRecognizer *)recognizer;
{
    NSLog(@"tapped");
...
//        // Add view controller subviews
    ...
    switch (style)
    {
        ...
        case Waiting:
            viewColor = UIColorFromHEX(0x0094DA);
            break;
    ...
    if([subTitle stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]].length > 0)
    {
        ...
        {
            self.viewText.font = [UIFont fontWithName:_bodyTextFontFamily size:15];
...
- (void)slideInFromTop
{
    if (SYSTEM_VERSION_LESS_THAN(@"7.0"))
    {
        ...
        UITapGestureRecognizer *gestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTap1:)];

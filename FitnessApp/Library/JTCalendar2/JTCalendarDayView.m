...
//#import "JTCalendarMonthWeekDaysView.m"
...
@interface JTCalendarDayView (){
   ...
   // JTCalendarMonthWeekDaysView *menuMonthWeekDayView;
    BOOL isSelected;
...
- (void)configureConstraintsForSubviews
{
    ...
    circleView.frame = CGRectMake(0, 5, sizeCircle-10, sizeCircle-10);
    circleView.center = CGPointMake(self.frame.size.width / 2., self.frame.size.height / 2.);
    circleView.layer.cornerRadius = sizeCircle / 3.;
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if(selected){
        if(!self.isOtherMonth){
            ...
            textLabel.textColor = [UIColor whiteColor];//[self.calendarManager.calendarAppearance dayTextColorToday];//[self.calendarManager.calendarAppearance dayTextColorSelected];
            
            dotView.color = [self.calendarManager.calendarAppearance dayTextColorToday];//[self.calendarManager.calendarAppearance dayDotColorSelected];
        }
        else{
            circleView.color = [self.calendarManager.calendarAppearance dayCircleColorSelectedOtherMonth];
            textLabel.textColor = [self.calendarManager.calendarAppearance dayTextColorToday];//[self.calendarManager.calendarAppearance dayTextColorSelectedOtherMonth];
            dotView.color = [self.calendarManager.calendarAppearance dayTextColorToday];//[self.calendarManager.calendarAppearance dayDotColorSelectedOtherMonth];
        }
        
        circleView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 0.5, 0.5);
        tr = CGAffineTransformIdentity;
    }
    else if([self isToday]){
        if(!self.isOtherMonth){
            //Todays Circle View
            circleView.color = [UIColor colorWithRed:21./256. green:188./256. blue:211./256. alpha:1.];//[self.calendarManager.calendarAppearance dayCircleColorToday];
            textLabel.textColor = [UIColor whiteColor];//[self.calendarManager.calendarAppearance dayTextColorToday];
            dotView.color =  [UIColor blackColor];//[self.calendarManager.calendarAppearance dayDotColorToday];
                  }
        else{
        ...
        if(!self.isOtherMonth){
            textLabel.textColor = [self.calendarManager.calendarAppearance dayTextColorToday];//[self.calendarManager.calendarAppearance dayTextColor];
            dotView.color = [self.calendarManager.calendarAppearance dayTextColorToday];//[self.calendarManager.calendarAppearance dayDotColor];
        }
        else{
            
            //color of other month's day
            textLabel.textColor = [UIColor colorWithRed:171./256. green:171./256. blue:171./256. alpha:1.];//[self.calendarManager.calendarAppearance dayTextColorOtherMonth]; //[self.calendarManager.calendarAppearance dayTextColorToday];
            dotView.color = [self.calendarManager.calendarAppearance dayTextColorToday];//[self.calendarManager.calendarAppearance dayDotColorOtherMonth];
        }
...
- (void)reloadAppearance
{
    ...
    backgroundView.backgroundColor = [UIColor clearColor];//[UIColor colorWithRed:247./256. green:247./256. blue:247./256. alpha:1.];//self.calendarManager.calendarAppearance.dayBackgroundColor;
